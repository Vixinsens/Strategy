...
namespace Strategy.Domain.Tests
{
    ...
    public class GameControllerTests
    {
        ...
        public void MoveUnit_AllTypes()
        {
            ...
            Assert.AreEqual(movePositionX, archer.Coordinates.X);
            Assert.AreEqual(movePositionY, archer.Coordinates.Y);

            gameController = new GameController(CreateMap());
            var catapult = new Catapult(player, 14, 14);
            gameController.MoveUnit(catapult, movePositionX, movePositionY);
            Assert.AreEqual(movePositionX, catapult.Coordinates.X);
            Assert.AreEqual(movePositionY, catapult.Coordinates.Y);

            gameController = new GameController(CreateMap());
            var horseman = new Horseman(player, 14, 14);
            gameController.MoveUnit(horseman, movePositionX, movePositionY);
            Assert.AreEqual(movePositionX, horseman.Coordinates.X);
            Assert.AreEqual(movePositionY, horseman.Coordinates.Y);

            gameController = new GameController(CreateMap());
            var swordsman = new Swordsman(player, 14, 14);
            gameController.MoveUnit(swordsman, movePositionX, movePositionY);
            Assert.AreEqual(movePositionX, swordsman.Coordinates.X);
            Assert.AreEqual(movePositionY, swordsman.Coordinates.Y);
        }
        ...
        public void CanAttackUnit_Archer(int x, int y, bool canAttack)
        {
            ...
            var map = CreateMap(units: new Unit[] { archer, target });
            var gameController = new GameController(map);
        ...
        private static Map CreateMap(IReadOnlyList<Ground> ground=null, IReadOnlyList<Unit> units=null)
        {
            Ground[] arr_ground = new Ground[2500];
            for (int i = 1; i < 51; i++)
            {
                for (int j = 1; j < 51; j++)
                {
                    Ground g = new Grass(i, j);
                    arr_ground[(i - 1) * 50 + (j - 1)] = g;
                }
            }
            return new Map(ground ?? arr_ground, units ?? new Unit[0]);
        }
    }
}